...
|---|---|---|---|
| P0 PTB-XL pipeline | [EXPLICIT] | `ptbxl_week1_config.json`; checksum-before-parse/path-confined downloader; patient/SCP manifest builder; train-only per-lead normalization; strict verifier | 21,799 records, 18,869 patients, `(1000,12)`, fixed lead order, 61,007 SCP assignments, no leakage, 43,601 files checksummed |
...
| P0 false PASS prevention | [EXPLICIT] | common non-empty/count/file requirements, explicit exceptions, cached-file checksum validation, and fail-fast orchestration | 24 negative cases cover empty MIT/PTB-XL, missing records/headers/data/annotation, corrupt cached metadata, and wrong waveform checksums under normal Python and `python -O` |
