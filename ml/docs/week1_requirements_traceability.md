...
| P0 false PASS prevention | [EXPLICIT] | common non-empty/count/file requirements, exact PTB-XL manifest ID-set/row validation, explicit exceptions, cached-file checksum validation, and fail-fast orchestration | 26 negative cases cover empty MIT/PTB-XL, missing records/headers/data/annotation, duplicate/missing PTB-XL manifest IDs, corrupt cached metadata, and wrong waveform checksums under normal Python and `python -O` |
